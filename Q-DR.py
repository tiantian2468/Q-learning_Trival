...
# 设置随机种子
random.seed(43)  # 设置 Python 的随机种子
np.random.seed(41)  # 设置 Numpy 的随机种子
...
# # 景点信息：[(推荐度, 游览时长, 花费)]
# spot_info = [
#     (9, 2, 15),   # 景点1: 推荐度 9, 游览时长 2小时, 花费 15元
#     (7, 3, 10),   # 景点2: 推荐度 7, 游览时长 3小时, 花费 10元
#     (8, 1, 5),    # 景点3: 推荐度 8, 游览时长 1小时, 花费 5元
#     (6, 4, 20),   # 景点4: 推荐度 6, 游览时长 4小时, 花费 20元
#     (10, 5, 25),  # 景点5: 推荐度 10, 游览时长 5小时, 花费 25元
#     (5, 2, 8),    # 景点6: 推荐度 5, 游览时长 2小时, 花费 8元
#     (8, 3, 18),   # 景点7: 推荐度 8, 游览时长 3小时, 花费 18元
#     (7, 1, 6),    # 景点8: 推荐度 7, 游览时长 1小时, 花费 6元
#     (9, 3, 22),   # 景点9: 推荐度 9, 游览时长 3小时, 花费 22元
#     (6, 2, 12)    # 景点10: 推荐度 6, 游览时长 2小时, 花费 12元
# ]

# 通勤时间矩阵（景点之间的通勤时间，假设单位是小时）
# commute_time = [
#     [0, 0.5, 1, 1.5, 2, 1, 0.8, 1.2, 1.3, 1.1, 0.7],  # 从景点0（酒店）到其他景点的通勤时间
#     [0.5, 0, 1.2, 1, 1.5, 0.7, 1, 0.6, 1, 1.3, 0.9],
#     [1, 1.2, 0, 0.7, 1.1, 1.2, 0.5, 1.4, 1, 1.1, 0.8],
#     [1.5, 1, 0.7, 0, 1.3, 0.9, 1.2, 1.1, 1, 1.4, 1],
#     [2, 1.5, 1.1, 1.3, 0, 1.7, 1.3, 1.5, 1.6, 2, 1.2],
#     [1, 0.7, 1.2, 0.9, 1.7, 0, 0.9, 1.3, 1, 1.5, 1.1],
#     [0.8, 1, 0.5, 1.2, 1.3, 0.9, 0, 0.7, 1, 1, 0.6],
#     [1.2, 0.6, 1.4, 1.1, 1.5, 1.3, 0.7, 0, 1.2, 1.4, 1.1],
#     [1.3, 1, 1, 1, 1.6, 1, 1, 1.2, 0, 1.5, 1.2],
#     [1.1, 1.3, 1.1, 1.4, 2, 1.5, 1, 1.4, 1.5, 0, 1.3],
#     [0.7, 0.9, 0.8, 1, 1.2, 1.1, 0.6, 1.1, 1.2, 1.3, 0]
# ]
commute_time = [
...
# 初始化旅游规划者
planner = TouristPlanner(num_spots=10, spot_info=spot_info, max_time=max_time, commute_time=commute_time)

# 训练模型
planner.train(episodes=1000)

# 获取最佳游览顺序
best_sequence = planner.get_best_sequence()

# 打印最佳旅游顺序
print("最佳旅游顺序（景点编号）：", best_sequence)
# 最大游览时间
max_time = 10
